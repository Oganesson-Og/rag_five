...
class PDFExtractor(BaseExtractor):
    ...
    async def extract(self, document: 'Document') -> 'Document':
        """
        Main extraction method that must be implemented as required by BaseExtractor.
        This orchestrates the entire PDF extraction process.
        """
        try:
            # Open the PDF document
            doc = fitz.open(document.source)
            
            # Determine document type (native vs scanned)
            is_scanned = self._check_if_scanned(doc)
            document.doc_info['is_scanned'] = is_scanned
            
            if is_scanned and self.has_ocr:
                # Handle scanned PDFs with OCR
                content = await self._process_scanned_pdf(doc)
            else:
                # Handle native PDFs with direct text extraction
                content = await self._process_native_pdf(doc)
            
            # Extract tables regardless of PDF type
            tables = await self._extract_tables_with_structure(doc)
            if tables:
                document.doc_info['tables'] = tables
            
            # Extract images and diagrams
            images = await self._extract_images_with_context(doc)
            if images:
                document.doc_info['images'] = images
                
                # Process diagrams if any images are classified as diagrams
                diagrams = [
                    img for img in images 
                    if self._is_diagram(img.get('data'))
                ]
                if diagrams:
    ...
    def _init_ocr(self):
        ...
        try:
            ...
            self.ocr = OCR(
                languages=['en'],  # Configure based on your needs
    ...
    async def _perform_ocr(self, doc: fitz.Document) -> str:
        ...
        for page in doc:
            try:
                ...
                # Perform OCR
                page_text = await self.ocr.extract_text(
                    img,
