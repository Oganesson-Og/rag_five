...
class MathDiagramGenerator:
    ...
    def f4_gtg_we4_velocity_time_graph(self):
        ...
        # Annotations
        ax.text(2, 4, 'Accel', ha='center', color='m', rotation=63)
        ax.text(7, 8.3, 'Const Vel', ha='center', color='m')
        ax.text(11, 4, 'Decel', ha='center', color='m', rotation=-76)
    ...
    def f4_ai_we1_inequality_graph(self):
        ...
        y_bound = (6 - x_range) / 2
        ax.plot(x_range, y_bound, 'blue', linestyle='-', label='x + 2y = 6')
        # Shade unwanted (y > (6-x)/2)
        ax.fill_between(x_range, y_bound, 5, where=y_bound<=5, color='blue', alpha=0.2, label='x+2y>6 unw')

        # Implicit x>=0, y>=0 (already limited by axes)

        # Vertices: (0,0), (6,0), (0,3)
        ax.plot([0, 6, 0], [0, 0, 3], 'ko')
        ax.text(0.1, -0.2, '(0,0)')
        ax.text(6, 0.2, '(6,0)')
        ax.text(-0.5, 3, '(0,3)')
        ...
        ax.set_xlabel("x-axis")
        ax.set_ylabel("y-axis")
        ax.set_title("Graph of x >= 0, y >= 0, x + 2y <= 6")
        ax.legend(fontsize=8, loc='upper right')
    ...
    def f4_gpc_notes_circle_theorems_1_5(self):
        ...
        ax.text(O[0], O[1]+0.5, '∠AOB', color='red')
        ...
        ax.text(C[0], C[1]-0.5, '∠ACB', color='blue')
        ...
        ax.text(C[0]-0.5, C[1], '∠ACB', color='blue')
        ax.text(D[0]+0.5, D[1], '∠ADB', color='green')
        ax.set_xlim(-radius-1, radius+1); ax.set_ylim(-radius-1, radius+1)
        ...
        ax.add_patch(rect_patch)
        ax.text(C[0]-0.5, C[1]-0.5, '90°', color='red')
        ax.set_xlim(-radius-1, radius+1); ax.set_ylim(-radius-1, radius+1)
    ...
    def f4_gpc_notes_cyclic_quad_theorems(self):
        ...
        D = (radius*math.cos(math.radians(angles[3])), radius*math.sin(math.radians(angles[3])))
        verts = [A, B, C, D]
        labels = ['A', 'B', 'C', 'D']
        ...
        ax1.text(A[0], A[1]+0.3, '∠A', color='red')
        ax1.text(C[0], C[1]-0.4, '∠C', color='red')
        ax1.text(B[0]-0.4, B[1], '∠B', color='green')
        ax1.text(D[0]+0.4, D[1], '∠D', color='green')
        ...
        ax2.text(A[0]+0.1, A[1]+0.4, '∠BAD', color='red')
    ...
    def f4_gpc_we5_tangent_radius_angle(self):
        ...
        C = (radius*math.cos(math.radians(C_angle)), radius*math.sin(math.radians(C_angle)))
        O = center
        # Tangent at T (vertical line x = -radius)
        tan_x = [T[0], T[0]]
        tan_y = [T[1]-1.5, T[1]+1.5]
        A = (tan_x[0], tan_y[1]+ 1) # Point A on tangent above T

        circle = patches.Circle(center, radius, fill=False); ax.add_patch(circle)
        ax.plot([O[0], T[0]], [O[1], T[1]], 'r-', label='Radius OT') # OT
        ax.plot([O[0], C[0]], [O[1], C[1]], 'r-', label='Radius OC') # OC
        ax.plot(tan_x, tan_y, 'b-', label='Tangent AT') # Tangent AT
    ...
    def f4_gcl_we2_locus_perp_bisector_construct(self):
        ...
        # Intersection points (calculated for plotting line)
        # Midpoint is (dist_ab/2, 0) = (3,0)
        # Locus is vertical line x = 3
        locus_x = dist_ab/2
        ax.plot([locus_x, locus_x], [-compass_radius, compass_radius], 'b-', linewidth=2, label='Locus (Perpendicular Bisector)')

        ax.set_xlim(-1, dist_ab+1); ax.set_ylim(-compass_radius-0.5, compass_radius+0.5)
    ...
    #     # filename = inspect.stack()[0][3] # Reverted this line
    #     filename = self.get_filename_from_caller() # MODIFIED: Use the original method
    #     fig, ax = plt.subplots(figsize=(6, 6), facecolor='white')
    #     ax.set_facecolor('white'); ax.set_aspect('equal'); ax.axis('off')
    #     dist_ab = 5
    #     rA = 3
    ...
    #     # Using lightgrey now, with moderate alpha. zorder=0 ensures it's behind other elements.
    ...
    #                        Patch(facecolor='lightgrey', alpha=0.7, label='Shaded Region\n(PA ≤ 3 AND PB ≤ 4)')] # Updated Patch color
    #     ax.legend(handles=legend_elements, loc='upper right', fontsize=8)

    #     # Call the save plot method - check console for "Saving plot..." message
    #     return self._save_plot(fig, filename)

    def f4_gcl_we5_locus_intersection(self):
        # filename = self.get_filename_from_caller() # This caused the error
        filename = inspect.stack()[0][3] # MODIFIED: Get current function name directly
        fig, ax = plt.subplots(figsize=(6, 6), facecolor='white')
        ax.set_facecolor('white'); ax.set_aspect('equal'); ax.axis('off')
        dist_ab = 5
        rA = 3
    ...
    #     r=10
    #     q = r * math.sin(math.radians(40)) / math.sin(math.radians(60)) 
    #     p = r * math.sin(math.radians(80)) / math.sin(math.radians(60)) 
    #     # Define vertices. Let R=(0,0). Let Q be on x-axis. Q=(p,0)
    #     R=(0,0); Q=(p,0)
    #     # P coords: Use cosine rule on QRP: q^2 = p^2+r^2 - 2pr cosQ
    #     # Or use coordinates from R: P = (q*cos(R_angle), q*sin(R_angle)). Need angle at R which is 60.
    #     # P = (q*cos(60), q*sin(60))? No, angle R is internal.
    #     # Place Q at origin. R is at (p,0). P needs angles. Angle PQR=40.
    #     Q=(0,0); R=(p,0)
    #     # P is such that PQ=r=10. Angle PQR=40.
    #     Px = r * math.cos(math.radians(180-40)) # Angle from pos x-axis if Q is origin
    #     Py = r * math.sin(math.radians(180-40))
    #     # Wait, easier: Place R at origin. Q is at (p,0). P makes angle 60 deg with QR.
    #     R=(0,0); Q=(p,0); P=(q*math.cos(math.radians(60)), q*math.sin(math.radians(60)))

    #     self._draw_triangle(ax, P, Q, R, labels={'A':'P', 'B':'Q', 'C':'R'},
    #                        sides={'a':'p', 'b':'q=?', 'c':'r=10'}, angles={'A':80,'B':40,'C':60})

    #     ax.set_xlim(min(P[0],Q[0],R[0])-1, max(P[0],Q[0],R[0])+1); ax.set_ylim(min(P[1],Q[1],R[1])-1, max(P[1],Q[1],R[1])+1)
    ...
    def f4_ttr_we5_3d_trig(self):
        ...
        R = np.array([10, 15, 0])  # Observer 2 (QR=10, angle PQR=90)
        # print(f"Point Coordinates: P={P}, Q={Q}, R={R}") # Keep commented unless needed
        ...
        # print(f"Point Coordinates: T={T}") # Keep commented unless needed
        ...
        # --- Revised Point Labels ---
        # Use ha='center', Z=0.5, va='bottom', adjust X/Y offsets
        p_label_coords = (P[0] - 0.5, P[1] - 0.5, 0.5) # Near P
        # print(f"Attempting to place 'P' label at: {p_label_coords}")
        ax.text(p_label_coords[0], p_label_coords[1], p_label_coords[2], 'P', color='black', ha='center', va='bottom', fontsize=9) # Centered

        q_label_coords = (Q[0] - 0.5, Q[1] + 0.5, 0.5) # Near Q
        # print(f"Attempting to place 'Q' label at: {q_label_coords}")
        ax.text(q_label_coords[0], q_label_coords[1], q_label_coords[2], 'Q', color='black', ha='center', va='bottom', fontsize=9) # Centered

        r_label_coords = (R[0] + 0.5, R[1] + 0.5, 0.5) # Near R
        # print(f"Attempting to place 'R' label at: {r_label_coords}")
        ax.text(r_label_coords[0], r_label_coords[1], r_label_coords[2], 'R', color='black', ha='center', va='bottom', fontsize=9) # Centered

        # Keep T label as preferred
        t_label_coords = (T[0], T[1]+0.4, T[2]+ 0.4)
        # print(f"Attempting to place 'T' label at: {t_label_coords}")
        ax.text(t_label_coords[0], t_label_coords[1], t_label_coords[2], 'T', color='black', ha='left', va='bottom', fontsize=9) # User preference


        # --- Adjusted Angle Labels (Working) ---
        tqp_angle_coords = (Q[0] + 1.5, Q[1] - 1.5, 1.5)
        # print(f"Attempting to place '{angle_tqp_deg:.0f}°' label at: {tqp_angle_coords}")
        ax.text(tqp_angle_coords[0], tqp_angle_coords[1], tqp_angle_coords[2], f'{angle_tqp_deg:.0f}°', color='g', ha='center', va='center', fontsize=9)

        trp_angle_coords = (R[0] - 1.5, R[1] - 1.5, 2.0)
        # print(f"Attempting to place 'θ≈{angle_trp_deg:.0f}°' label at: {trp_angle_coords}")
        ax.text(trp_angle_coords[0], trp_angle_coords[1], trp_angle_coords[2], f'θ≈{angle_trp_deg:.0f}°', color='r', ha='center', va='center', fontsize=9)

        pqr_angle_coords = (Q[0] + 1.5, Q[1] - 1.5, 0.5)
        # print(f"Attempting to place '90°' label at: {pqr_angle_coords}")
        ax.text(pqr_angle_coords[0], pqr_angle_coords[1], pqr_angle_coords[2], '90°', color='k', ha='left', va='bottom', fontsize=9)

        # --- Axes and Title ---
        ax.set_xlabel("X (e.g., East)")
        ax.set_ylabel("Y (e.g., North)")
        ax.set_zlabel("Z (Height)")
        ax.set_title("3D Trigonometry Problem")
    ...
    def f4_vo_notes_parallelogram_vectors(self):
        ...
        ax.arrow(D[0], D[1], diag_db[0], diag_db[1], head_width=0.2, head_length=0.3, fc='purple', ec='purple', length_includes_head=True) # DB
        ...
        ax.text(D[0]+diag_db[0]/2+0.1, D[1]+diag_db[1]/2, 'u-v', color='purple', fontweight='bold')
    ...
    def f4_vo_we1_parallelogram_vectors(self):
        ...
        ax.text(C[0]/2-0.1, C[1]/2, 'c', color='red', fontweight='bold')
    ...
    def f4_vo_we4_triangle_vectors_ratio(self):
        ...
        # Labels
        ax.text(A[0]/2, A[1]/2+0.1, 'a', color='blue', fontweight='bold')
        ax.text(B[0]/2+0.1, B[1]/2, 'b', color='red', fontweight='bold')
